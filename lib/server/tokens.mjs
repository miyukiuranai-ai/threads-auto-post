...
// 延長すると再び60日になる。失効してしまうと取り直しが必要なので、毎日確かめて早めに延ばす。
import { getDb, COLLECTIONS } from './firebase.mjs';
import { refreshLongLivedToken, shortError } from './threads.mjs';
...
export async function refreshTokens({ all = false } = {}) {
  ...
  for (const account of accounts) {
    ...
    if (left !== null && left < 0) {
      results.push({ account: account.name, result: 'expired', reason: 'トークンが失効済み。設定画面で入れ直してください' });
      continue;
    }
    ...
    try {
      ...
      await db.collection(COLLECTIONS.accounts).doc(account.id).set(
        { accessToken: res.access_token, tokenExpiresAt: expiresAt, tokenRefreshedAt: new Date().toISOString() },
        { merge: true }
      );
      results.push({ account: account.name, result: 'refreshed', daysLeft: daysLeft(expiresAt) });
    } catch (err) {
      results.push({ account: account.name, result: 'failed', reason: shortError(err) });
    }
  }

  const bad = results.filter((r) => r.result === 'failed' || r.result === 'expired');
  await db.collection(COLLECTIONS.runs).add({
    job: 'refresh-tokens',
    startedAt,
    finishedAt: new Date().toISOString(),
    status: bad.length ? 'failed' : 'ok',
    message: bad.map((r) => `${r.account}: ${r.reason}`).join(' / '),
    results,
  });
