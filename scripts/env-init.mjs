...
// npm run env:init -- --sa serviceAccount.json [--bucket xxx.firebasestorage.app] [--admin-user admin --admin-password xxx]
...
if (args['app-url']) set('APP_URL', String(args['app-url']));
...
const lines = ['# threads-auto-post の環境変数（git に含めない）', ...keys.map((k) => `${k}=${next[k]}`)];
fs.writeFileSync(envPath, lines.join('\n') + '\n');

const missing = ['FIREBASE_PROJECT_ID', 'FIREBASE_CLIENT_EMAIL', 'FIREBASE_PRIVATE_KEY', 'ADMIN_PASSWORD'].filter((k) => !next[k]);
