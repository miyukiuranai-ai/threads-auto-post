 ...
 * 何も起きなかった回も記録される印を見るので、投稿が無い日でも止まって見えない。
 */
export const lastRunAt = cache(
  unstable_cache(
    async () => {
      try {
        const snap = await getDb().collection(COLLECTIONS.meta).doc('tick').get();
        const at = snap.exists ? snap.data().lastAt : null;
        if (at) return at;
      } catch {
        // 印が読めなくても、実行ログから拾えることがある
      }
      const snap = await getDb().collection(COLLECTIONS.runs).orderBy('startedAt', 'desc').limit(1).get();
      return snap.docs[0]?.data().finishedAt ?? null;
    },
    ['last-run'],
    { revalidate: 20 }
  )
